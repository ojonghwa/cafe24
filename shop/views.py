...
def product_detail(request, id, slug):
    ...
    all_grade = Like.objects.filter(product=product).aggregate(Avg('grade'), Count('grade'))
    #print("all_grade: " + str(all_grade), file=sys.stdout)

    return render(request, 'shop/product/detail.html',
                {'product': product, 'grade': grade, 'all_grade': all_grade, 'like_form': like_form, 'post_list': page_object, 
...
def profile_create(request):
    ...
    return render(request, 'shop/customer/profile_form.html', {'form': form, 'profile': profile})
...
def ordered_list(request):
    page = request.GET.get('page','1')      #기본 페이지 1, ?page=1
    order_list = Order.objects.filter(user=request.user.username, paid=True).all()  #비용 지급한 목록만
    ...
    page_object = paginator.get_page(page)
    return render(request, 'shop/customer/ordered.html', 
...
def product_like(request, product_id):
    ...
    if like_product.exists():
        ...
        Like.objects.create( user=request.user, product=product, grade=int(request.POST['grade']) )
