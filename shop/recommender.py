...
def get_recommendations(id, max_results=3):
    ...
    # orders_querysetlist = list(OrderItem.objects.filter(product_id=id))
    # print(orders_querysetlist[1].product, file=sys.stdout)
    ...
    count_list = []
    [
        count_list.append(product_ids.count(i) / num_orders_for_product)
    ...
    dataframe = DataFrame(
        dataframe.sort_values("frequency", ascending=False).head(max_results)
    )
