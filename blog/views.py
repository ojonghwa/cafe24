...
def index(request):
    ...
    if so == 'recommend':       # 정렬
        post_list = Post.objects.annotate(num_voter=Count('voter')).order_by('-num_voter', '-created')
    else:       # recent
    ...
    if kw:      # 검색
        post_list = post_list.filter(
            ...
            Q(body__icontains=kw)  |        # 내용검색
            Q(author__username__icontains=kw)     # 작성자검색
        ).distinct()
    ...
    if kw:      # 검색
        products = products.filter(
            ...
            Q(description__icontains=kw)    # 내용검색
        ).distinct()
    else:
        products = None

    return render(request, 'blog/post_list.html', 
        { 'post_list': page_object, 'page': page, 'kw': kw, 'so': so, 'products': products })



def detail(request, post_id):
    post = get_object_or_404(Post, id=post_id)
    return render(request, 'blog/post_detail.html', { 'post': post, 'product': post.product })
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            return redirect('{}#comment_{}'.format(
                resolve_url('blog:detail', post_id=comment.post.id), comment.id))   # 스크롤 처리 앵커
    else:
        form = CommentForm()
